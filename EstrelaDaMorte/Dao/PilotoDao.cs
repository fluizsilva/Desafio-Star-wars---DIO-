using EstrelaDaMorte.Entidades;
using EstrelaDaMorte.Extensoes;
...
namespace EstrelaDaMorte.Dao
{
    public class PilotoDao : DaoBase
    {
        ...
        public async Task<List<Piloto>> ObterPorNomeLike(string nome)
        {
            ...
            if (nome.Length != 0 )
            {
                comando = $"select * from Pilotos where nome like '%{nome.Replace(' ', '%')}%'";
            }
            else
            {
                comando = $"select * from Pilotos order by nome asc";
            }
